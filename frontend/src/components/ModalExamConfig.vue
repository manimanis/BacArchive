<template>
  <div class="overlay" @click.self="$emit('close')">
    <div class="modal modal-wide" @click.stop>
      <div class="modal-hdr">
        <span>📅 Configuration des Examens — Année {{ annee }}</span>
        <button @click="$emit('close')" title="Fermer">✕</button>
      </div>

      <div class="modal-body modal-body-scroll">
        <div v-if="loading" class="hint center">⏳ Chargement…</div>
        ...
        <template v-else>
          ...
          <div class="exam-toolbar">
            <div class="field-row">
              ...
              <input type="text" min="0" max="20" :value="cfg.nbre_jours" :readonly="true" />
            </div>
            <div class="exam-toolbar-actions">
              ...
              <button class="btn-sm" @click="duplicateJour(activeJour)"
                :disabled="cfg.jours.length === 0 || cfg.jours.length >= 20"
                title="Dupliquer le jour actif">⎘ Dupliquer</button>
              <button class="btn-sm btn-danger" @click="removeJour(activeJour)"
                :disabled="cfg.jours.length === 0"
          ...
          <div v-if="cfg.jours.length > 0" class="exam-tabs-bar">
            <button v-for="(jour, di) in cfg.jours" :key="di"
              class="exam-tab" :class="{ active: activeJour === di }"
              @click="activeJour = di"
              :title="`Jour ${di + 1} — ${jour.examens.length} examen(s)`">
              <span class="exam-tab-num">J{{ di + 1 }}</span>
          ...
          <div v-if="cfg.jours.length > 0" class="exam-tab-content">
            <div class="exam-jour-header">
              ...
              <div class="exam-jour-actions">
                <button class="btn-sm" @click="moveJour(activeJour, -1)" :disabled="activeJour === 0" title="Monter ce jour">↑</button>
                <button class="btn-sm" @click="moveJour(activeJour, 1)" :disabled="activeJour >= cfg.jours.length - 1" title="Descendre ce jour">↓</button>
              </div>
            </div>

            <div class="field-row">
              <label>Examens du jour</label>
              <input type="text" min="0" max="10" :value="currentJour.nb_examens" :readonly="true"/>
              <button class="btn-sm" @click="addExamen(activeJour)"
                :disabled="currentJour.examens.length >= 10"
            ...
            <div v-for="(exam, ei) in currentJour.examens" :key="ei" class="exam-examen">
              <div class="exam-examen-hdr">
                ...
                <div class="exam-jour-actions">
                  <button class="btn-sm" @click="moveExamen(activeJour, ei, -1)" :disabled="ei === 0" title="Monter">↑</button>
                  ...
                  <button class="btn-sm btn-danger" @click="removeExamen(activeJour, ei)" title="Supprimer cet examen">🗑</button>
                </div>
              </div>
              ...
              <div class="field-row">
                ...
                <select :value="exam.section" @change="setExamField(activeJour, ei, 'section', $event.target.value)">
                  ...
                  <option v-for="s in sectionsList" :key="s.value" :value="s.value">{{ s.label }}</option>
                </select>
              </div>
              ...
              <div class="field-row">
                <label>Nb séances</label>
                <input type="text" min="0" max="10" :value="exam.nb_seances"
                  :readonly="true"/>
                <button class="btn-sm" @click="addSeance(activeJour, ei)"
                  :disabled="exam.seances.length >= 10"
              ...
              <div v-for="(se, si) in exam.seances" :key="si" class="exam-seance">
                ...
                <button class="btn-sm btn-danger" @click="removeSeance(activeJour, ei, si)" title="Supprimer">✕</button>
              </div>
            </div>
          </div>
        </template>
      ...
      <div class="modal-ftr">
        <span class="modal-ftr-info" v-if="cfg.file">📄 {{ cfg.file }}</span>
...
// Garantit que l'onglet actif reste valide après ajouts/suppressions
...
// Récupère la date formatée du premier examen du jour (pour l'onglet)
...
const setNbreJours = (n) => {
  n = Math.max(0, Math.min(20, n))
  cfg.nbre_jours = n
  // Étendre ou réduire la liste
  while (cfg.jours.length < n) cfg.jours.push(newJour(props.annee))
  while (cfg.jours.length > n) cfg.jours.pop()
  clampActiveJour()
}

const addJour = () => {
  if (cfg.jours.length >= 20) return
  cfg.jours.push(newJour(props.annee))
  cfg.nbre_jours = cfg.jours.length
  // Aller sur le nouveau jour
  activeJour.value = cfg.jours.length - 1
}
...
const duplicateJour = (di) => {
  ...
  // Copie profonde des examens (y compris séances)
  const copy = {
  ...
  // Incrémenter la date dupliquée d'un jour (si elle existe)
...
const moveJour = (di, delta) => {
  ...
  cfg.jours[ni] = tmp
  // Mettre à jour l'onglet actif pour qu'il suive le jour déplacé
  if (activeJour.value === di) activeJour.value = ni
  else if (activeJour.value === ni) activeJour.value = di
}

const setNbExamens = (di, n) => {
  n = Math.max(0, Math.min(10, n))
  cfg.jours[di].nb_examens = n
  while (cfg.jours[di].examens.length < n) cfg.jours[di].examens.push(newExamen(props.annee))
  while (cfg.jours[di].examens.length > n) cfg.jours[di].examens.pop()
}

const addExamen = (di) => {
  if (cfg.jours[di].examens.length >= 10) return
  cfg.jours[di].examens.push(newExamen(props.annee))
...
const duplicateExamen = (di, ei) => {
  const newExamen = {...cfg.jours[di].examens[ei]}
  cfg.jours[di].examens.push(newExamen)
  cfg.jours[di].nb_examens = cfg.jours[di].examens.length
}
...
const setNbSeances = (di, ei, n) => {
  n = Math.max(0, Math.min(10, n))
  cfg.jours[di].examens[ei].nb_seances = n
  const seances = cfg.jours[di].examens[ei].seances
  while (seances.length < n) seances.push(newSeance())
  while (seances.length > n) seances.pop()
}
...
.exam-tab.active .exam-tab-badge {
  background: #fff;
  color: #1a3a5c;
}
